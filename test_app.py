...
async def test_open_wsl_button_present_and_does_not_crash(user: User) -> None:
    await user.open('/')
    await user.should_see('Cábula', retries=50)

    user.find(marker='open-wsl').click()
    ...
    assert user.notify.messages, 'esperava alguma notificacao ao clicar em Ligar ao WSL'
    print('OPEN WSL BUTTON OK (no crash)')
